...
google_play_data = pd.read_csv("GooglePlayStore.csv")  # replace with the actual file path
...
top_categories_app_store = highly_rated_app_store.groupby('Category')['Rating'].mean().sort_values(ascending=False)
top_categories_google_play = highly_rated_google_play.groupby('Category')['Rating'].mean().sort_values(ascending=False)
...
print(highly_rated_app_store[['App', 'Category', 'Rating', 'Reviews']].sort_values(by='Reviews', ascending=False).head(10))

print("\nHighly Rated and Popular Apps in Google Play Store:")
print(highly_rated_google_play[['App', 'Category', 'Rating', 'Installs', 'Reviews']].sort_values(by='Installs', ascending=False).head(10))
